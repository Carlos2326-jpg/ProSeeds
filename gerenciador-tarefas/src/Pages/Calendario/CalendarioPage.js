// ============================================================
// PAGE - Calendário (seguindo wireframe)
// Study+
// ============================================================
...
import {
  ...
  cronogramas as cronogramasIniciais,
  disciplinas,
...
const CalendarioPage = () => {
  ...
  const handleSalvar = () => {
    ...
    if (cronogramaEditando) {
      setCronogramas((prev) =>
        prev.map((c) =>
          c.id === cronogramaEditando.id ? { ...c, ...form } : c
        )
      );
    } else {
      criarCronograma({ ...form, usuario_id: 1 }, setCronogramas);
    }
    ...
    <div style={styles.wrapper}>
      <div style={styles.container}>
        ...
        <div style={styles.filtros}>
          ...
          <div style={styles.filtroLinha}>
            <input style={styles.inputFiltro} type="date" name="data_inicio" value={filtros.data_inicio} onChange={handleFiltro} placeholder="Data Início" />
            <input style={styles.inputFiltro} type="date" name="data_fim" value={filtros.data_fim} onChange={handleFiltro} placeholder="Data Término" />
            ...
            <div style={styles.listaItens}>
              {cronogramasFiltrados.length === 0 ? (
                ...
                cronogramasFiltrados.map((c) => {
                  const tarefa = tarefas.find((t) => t.id === c.tarefa_id);
                  ...
                  return (
                    <div key={c.id} style={styles.cronogramaCard}>
                      <div style={styles.cronogramaInfo}>
                        <span style={styles.cronogramaNome}>
                          {tarefa ? tarefa.titulo : "Sem tarefa"}
                        </span>
                        <span style={styles.cronogramaDetalhe}>
                          {disciplina ? disciplina.nome : ""} · {c.recorrencia}
                        </span>
                        <span style={styles.cronogramaDetalhe}>
                      ...
                      <div style={styles.cronogramaBotoes}>
                        <button style={styles.btnDeletar} onClick={() => excluirCronograma(c.id, setCronogramas)}>
                          Deletar
                        </button>
                        <button style={styles.btnEditar} onClick={() => handleEditar(c)}>
                          Editar
                        </button>
                      </div>
                    </div>
        ...
        <div style={styles.overlay}>
          <div style={styles.modal}>
            <div style={styles.modalHeader}>
              <span style={styles.modalTitulo}>
                {cronogramaEditando ? "Editar Cronograma" : "Criar Cronograma"}
              </span>
            </div>
            <div style={styles.modalForm}>
              ...
              <div style={styles.linhaModal}>
                <input style={styles.inputModal} type="time" name="hora_inicio" value={form.hora_inicio} onChange={handleFormChange} placeholder="Hora Início" />
                <input style={styles.inputModal} type="time" name="hora_fim" value={form.hora_fim} onChange={handleFormChange} placeholder="Hora Fim" />
              </div>
              ...
              <div style={styles.modalBotoes}>
                <button style={styles.btnCriarModal} onClick={handleSalvar}>
                  {cronogramaEditando ? "Salvar" : "Criar"}
                </button>
                <button style={styles.btnCancelarModal} onClick={() => { setModalAberto(false); setCronogramaEditando(null); }}>
                  Cancelar
                </button>
              </div>
            </div>
